...
public class DocumentScanner {
	 ...
	 *            The maximum fraction a row's height can be of the previous
	 *            row's height, in order for the new (short) row to be merged in
	 ...
	 *            The minimum width of a space, expressed as a fraction of the
	 *            height of a row of text.
	 */
	 ...
	 *            The minimum width of a character, expressed as a fraction of
	 ...
	 *            The minimum width of a character break (a vertical column of
	 *            whitespace that separates two characters on a row of text),
	 *            expressed as a fraction of the height of a row of text.
	 */
	...
	public final void scan(PixelImage pixelImage, DocumentScannerListener listener, int blockX1, int blockY1,
		 ...
		 * blackCount++; } ///System.out.println("left blackCount="+blackCount+
		 * " maxBlack="+maxBlack+" blockY1="+blockY1+" blockY2="+blockY2); if
		 * (blackCount >= maxBlack) { ///System.out.println("    reduce left");
		 * reduced = true; blockX1++; if (blockX1 >= blockX2) break; } // Right
		 * edge blackCount = 0; idx = (blockY1*w)+blockX2; maxBlack =
		 * Math.max(1, (int)((float)((blockY2+1)-blockY1)*blackElimFraction));
		 * for (int y = blockY1; y <= blockY2; y++, idx += w) {
		 * ///System.out.print("["+pixels[idx]+"]"); if (pixels[idx] < thresh)
		 * blackCount++; } ///System.out.println(); ///System.out.println(
		 * "right blackCount="+blackCount+" maxBlack="+maxBlack+" blockY1="
		 * +blockY1+" blockY2="+blockY2); if (blackCount >= maxBlack) {
		 * ///System.out.println("    reduce right"); reduced = true; blockX2--;
		 * if (blockX1 >= blockX2) break; } // Top edge blackCount = 0; idx =
		 * (blockY1*w)+blockX1; maxBlack = Math.max(1,
		 * (int)((float)((blockX2+1)-blockX1)*blackElimFraction)); for (int x =
		 * blockX1; x <= blockX2; x++, idx++) { if (pixels[idx] < thresh)
		 * blackCount++; } ///System.out.println("top blackCount="+blackCount+
		 * " maxBlack="+maxBlack+" blockX1="+blockX1+" blockX2="+blockX2); if
		 * (blackCount >= maxBlack) { ///System.out.println("    reduce top");
		 * reduced = true; blockY1++; if (blockY1 >= blockY2) break; } // Bottom
		 * edge blackCount = 0; idx = (blockY2*w)+blockX1; maxBlack =
		 * Math.max(1, (int)((float)((blockX2+1)-blockX1)*blackElimFraction));
		 * for (int x = blockX1; x <= blockX2; x++, idx++) { if (pixels[idx] <
		 * thresh) blackCount++; } ///System.out.println("bottom blackCount="
		 * +blackCount+" maxBlack="+maxBlack+" blockX1="+blockX1+" blockX2="
		 * +blockX2); if (blackCount >= maxBlack) { ///System.out.println(
		 * "    reduce bottom"); reduced = true; blockY2--; if (blockY1 >=
		 * blockY2) break; } }
		 * 
		...
		for (int i = 0; (i + 4) <= al.size(); i += 2) {
			...
			int whiteH = bY2 - bY1;
			int row1H = bY3 - bY2;
			if (((row1H <= (int) (row0H * shortRowFraction)) || (row1H < 6))
					&& ((whiteH <= (int) (row0H * shortRowFraction)) || (whiteH < 6))) {
				al.remove(i + 2);
				al.remove(i + 1);
				i -= 2;
			}
	...
	private void processRow(PixelImage pixelImage, DocumentScannerListener listener, int[] pixels, int w, int h, int x1,
		...
		int rowHeight = y2 - y1;
		int minCharBreakWidth = Math.max(1, (int) (rowHeight * minCharBreakWidthAsFractionOfRowHeight));
		int liberalWhitspaceMinWhitePixelsPerColumn = (int) (rowHeight * liberalPolicyAreaWhitespaceFraction);
		...
		int minSpaceWidth = (int) (rowHeight * minSpaceWidthAsFractionOfRowHeight);
		// Next combine concecutive supposed character cells where their
		// leftmost X positions are too close together.
		int minCharWidth = (int) (rowHeight * minCharWidthAsFractionOfRowHeight);
		if (minCharWidth < 1) {
			minCharWidth = 1;
		}
		for (int i = 0; (i + 4) < al.size(); i += 2) {
