...
public class DocumentScanner
{
            ...
            int blockY2)
    {
        ...
        for (int i = 0; (i + 4) <= al.size(); i += 2)
        {
            ...
            int whiteH = bY2 - bY1;
            int row1H = bY3 - bY2;
            if (((row1H <= (int) (row0H * shortRowFraction))
                    || (row1H < 6))
                    && ((whiteH <= (int) (row0H * shortRowFraction))
                    || (whiteH < 6)))
            {
                al.remove(i + 2);
                al.remove(i + 1);
...
///System.err.println("process row: "+blockX1+","+bY1+" "+blockX2+","+bY2);
            ...
            int y2)
    {
        ...
        int minCharBreakWidth = Math.max(
                1,
                (int) (rowHeight * minCharBreakWidthAsFractionOfRowHeight));
        int liberalWhitspaceMinWhitePixelsPerColumn =
                (int) (rowHeight * liberalPolicyAreaWhitespaceFraction);
