...
public class ProcesadorEsqueleto extends ProcesadorImagen {
        ...
        do {
            ...
            for (int x = 0; x < imagenProcesada.getWidth(); x++) {
                for (int y = 0; y < imagenProcesada.getHeight(); y++) {
                    if (getP1(imagenProcesada, x, y) == 1
                            && vecinos(imagenProcesada, x, y) >= 2
                            && vecinos(imagenProcesada, x, y) <= 6
                            && transiciones(imagenProcesada, x, y) == 1
                            && (getP2(imagenProcesada, x, y)
                            * getP4(imagenProcesada, x, y)
                            * getP6(imagenProcesada, x, y)) == 0
                            && (getP4(imagenProcesada, x, y)
                            * getP6(imagenProcesada, x, y)
    ...
    private int transiciones(BufferedImage esqueleto, int x, int y) {
        int transiciones = 0;
        int vecinos[] = {getP2(esqueleto, x, y), getP3(esqueleto, x, y),
            getP4(esqueleto, x, y), getP5(esqueleto, x, y),
            getP6(esqueleto, x, y), getP7(esqueleto, x, y),
            getP8(esqueleto, x, y), getP9(esqueleto, x, y),
            getP2(esqueleto, x, y)
    ...
    private int vecinos(BufferedImage imagen, int x, int y) {
        return getP2(imagen, x, y) + getP3(imagen, x, y)
                + getP4(imagen, x, y) + getP5(imagen, x, y)
                + getP6(imagen, x, y) + getP7(imagen, x, y)
                + getP8(imagen, x, y) + getP8(imagen, x, y);
    }
    ...
    private int getP5(BufferedImage imagen, int x, int y) {
        return x + 1 > imagen.getWidth() - 1 || y - 1 < 0 ? 0
                : (imagen.getRGB(x + 1, y - 1) == BLANCO ? 0 : 1);
    }
    ...
    private int getP9(BufferedImage imagen, int x, int y) {
        return x - 1 < 0 || y + 1 > imagen.getHeight() - 1 ? 0
                : (imagen.getRGB(x - 1, y + 1) == BLANCO ? 0 : 1);
    }
