...
public class ProcesadorInclinacion extends ProcesadorImagen {
	...
	public BufferedImage procesar(BufferedImage imagen) {
        ...
        for (int b = -imagen.getWidth() + 1; !encontrado && b < imagen.getHeight(); b++) {
            for (int x = imagen.getWidth() - 1; !encontrado && x >= 0
                    && x + b >= 0 & x + b < imagen.getHeight(); x--) {
        ...
        //B2
        encontrado = false;
        for (int b = imagen.getHeight() - 1;
                !encontrado && imagen.getWidth() - 1 + b >= 0; b--) {
            for (int x = -b <= 0 ? 0 : -b;
                    !encontrado && x < imagen.getWidth()
                    && x + b >= 0 && x + b < imagen.getHeight(); x++) {
                if (imagen.getRGB(x, x + b) != BLANCO) {
                    b2 = b;
                    encontrado = true;
        ...
        for (int b = 0; !encontrado && b < imagen.getHeight() + imagen.getWidth() - 1; b++) {
            for (int x = b - imagen.getHeight() + 1 <= 0 ? 0 : b - imagen.getHeight() + 1;
                    !encontrado && x < imagen.getWidth() && -x + b >= 0 && -x + b < imagen.getHeight(); x++) {
                if (imagen.getRGB(x, -x + b) != BLANCO) {
                    b3 = b;
                    encontrado = true;
                }
        ...
        //B4
        encontrado = false;
        for (int b = imagen.getWidth() + imagen.getHeight() - 2;
                !encontrado && b >= 0; b--) {
            for (int x = b - imagen.getHeight() + 1 <= 0 ? 0 : b - imagen.getHeight() + 1;
                    !encontrado && x < imagen.getWidth() && -x + b >= 0; x++) {
                if (imagen.getRGB(x, -x + b) != BLANCO) {
                    b4 = b;
                    encontrado = true;
                }
